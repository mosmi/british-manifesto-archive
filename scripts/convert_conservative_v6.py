#!/usr/bin/env python3
"""Convert Conservative Party Manifesto 2024 PDF to Markdown - v6.
Key: Split chars into columns by grouping chars at same y into words,
then assign words to columns based on word-start x.
"""
import pdfplumber
...
def chars_to_words(chars):
    ...
    for ch in sorted_chars[1:]:
        ...
        # Gap between chars: if > ~3pt, it's a new word
...
def extract_column_lines(chars, split_x=None, col='all'):
    """
    Extract lines from chars, splitting into columns by word-start position.
    Within each y-bucket, words whose start x < split_x go to left col,
    words whose start x >= split_x go to right col.
    """
    lines_dict = defaultdict(list)
    for ch in chars:
        y_key = round(ch['y0'] / 2) * 2
