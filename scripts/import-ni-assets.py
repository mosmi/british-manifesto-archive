#!/usr/bin/env python3
"""
Import Northern Ireland Assembly election manifesto PDFs from the local source archive into
manifestos/stormont/{id}/{party}/...

Driven by scripts/ni-assets.json.

Usage:
  python3 scripts/import-ni-assets.py
  python3 scripts/import-ni-assets.py --dry-run
"""
...
MANIFEST = Path(__file__).resolve().parent / "ni-assets.json"
DEST_ROOT = ROOT / "manifestos" / "stormont"
...
def postprocess_cover(src_png: Path, out_png: Path, mode: str, pil_crop: list[float] | None = None) -> None:
    ...
    if mode == "cropmarks":
        ...
        panel = panel.resize((COVER_WIDTH, PORTRAIT_H), Image.Resampling.LANCZOS)
        result = panel.convert("RGBA")
    elif mode == "third-right":
        third_w = img.width // 3
        panel = img.crop((2 * third_w, 0, img.width, img.height))
        result = fit_portrait(panel)
    elif mode == "two-thirds-right":
        third_w = img.width // 3
        panel = img.crop((third_w, 0, img.width, img.height))
        result = fit_portrait(panel)
    elif mode == "landscape":
...
def make_cover(pdf: Path, out_png: Path, mode: str = "page", pil_crop: list[float] | None = None, cover_src: Path | None = None) -> None:
    ...
    render_w = COVER_WIDTH * 3 if mode in {"spread-two-thirds-cropmarks", "third-right", "two-thirds-right"} else (
