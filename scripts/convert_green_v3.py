#!/usr/bin/env python3
"""Convert Green Party Manifesto 2024 PDF to Markdown - v3.
Fix: Use y_key rounding of 4pt to properly group bullet markers with their text.
"""
import pdfplumber
...
Y_BUCKET = 4  # Group chars within 4pt vertically (to handle font baseline differences)
...
def find_column_split(chars, min_x, max_x):
    ...
    for ch in chars:
        ...
        if ch['y0'] < 15 or ch['y0'] > 810:
            continue
        y_key = round(ch['y0'] / Y_BUCKET) * Y_BUCKET
...
def extract_column_lines(chars, col_min_x, col_max_x):
    ...
    for y_key in sorted(lines_dict.keys(), reverse=True):
        ...
        # Detect if this line has a bullet marker
        has_bullet = any(c['text'] == '•' for c in col_sorted)
        
        # Build text: join all chars but handle bullet properly
        ...
        # If has bullet, use bullet as the classifier
        if has_bullet:
            size = 13.0  # Bullet marker size
        else:
        ...
        # Determine if this is a bullet line
        if has_bullet:
            cls = 'bullet_line'
...
def lines_to_blocks(lines):
    ...
    for line in lines:
        ...
        # Continue previous bullet (small gap, same indentation)
        if blocks and blocks[-1]['type'] == 'bullet' and gap <= 20 and prev_cls == 'bullet':
            blocks[-1]['text'] += ' ' + clean
            prev_y0 = y0
            prev_cls = cls
            continue
        ...
        if current_para is None:
            ...
            # Merge (Green PDF uses ~15pt line spacing)
...
def process_page(page):
    """Process a PDF page (may be single or two-page spread)."""
    chars = page.chars
    if not chars:
        return []
