...
os.makedirs('data/sources/european-parliament-elections/westminster-to-ep', exist_ok=True)
...
# Load cached wikitext if present
cache_file = 'data/sources/european-parliament-elections/ep_pages_wikitext.json'
...
manual_compositions = {
    ...
    '1984': {
        ...
        'Cleveland & Yorkshire North': ['Cleveland', 'Middlesbrough', 'Redcar', 'Stockton North', 'Stockton South', 'Richmond (Yorks)', 'Thirsk and Malton', 'Scarborough'],
...
for yr, hex_data, hex_norm_map in [('1979', hex_79, map_79), ('1984', hex_83, map_83), ('1994', hex_97, map_97)]:
    ...
    for ep in eps:
        ...
        if yr in manual_compositions and ep in manual_compositions[yr]:
            matched_westminster = list(manual_compositions[yr][ep])
        else:
    ...
    # Nearest-neighbor spatial assignment for any unmapped GE hexes
    total_hexes = list(hex_data.keys())
    unmapped_hexes = [k for k in total_hexes if k not in hex_to_ep]
    mapped_hexes = [k for k in total_hexes if k in hex_to_ep]
    
    for un in unmapped_hexes:
        uq, ur = hex_data[un]['q'], hex_data[un]['r']
        best_dist = 999999
        best_match = None
        for m in mapped_hexes:
            mq, mr = hex_data[m]['q'], hex_data[m]['r']
            d = hex_dist(uq, ur, mq, mr)
            if d < best_dist:
                best_dist = d
                best_match = m
        if best_match:
            assigned_ep = hex_to_ep[best_match]
            hex_to_ep[un] = assigned_ep
            if un not in crosswalk[assigned_ep]:
                crosswalk[assigned_ep].append(un)

    # Compute averaged (q, r) centroids for 100% of EP constituencies
    centroids = {}
    for ep, seats in crosswalk.items():
    ...
    final_unmapped = [k for k in total_hexes if k not in hex_to_ep]
